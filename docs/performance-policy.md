...
The version-five-to-current migration suite reaches schema version 12 and measures installation of
the partial unique index, canonical publication-date and half-star-rating detail table, identifier
tables, seeded default identifier types, and fixed genre-membership table;
unit migration coverage separately supplies deliberate version-seven duplicates, verifies the
deterministic repair, and covers version-eight metadata, version-eleven identifier defaults, and
the fixed-genre schema. The query-only v3 fixture additionally assigns three identifiers per book and
